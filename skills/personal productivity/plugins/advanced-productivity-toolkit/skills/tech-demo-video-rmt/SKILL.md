...
#    NOTE: in this packaged copy the scaffold's code files are stored with a
#    trailing `.txt` (so the M365/Cowork packager accepts .tsx/.ts/.mjs). Strip
#    that `.txt` as we overlay them into the project.
( cd "$SKILL_DIR/scaffold" && find src scripts -type f | while read -r f; do
    dest="$SCRATCH/${f%.txt}"; mkdir -p "$(dirname "$dest")"; cp "$f" "$dest"
  done )
...
run the same `npx create-video --yes --blank --no-tailwind $SCRATCH`, then overlay
the scaffold, stripping the trailing `.txt` (see note above):
```powershell
Get-ChildItem "$SKILL_DIR\scaffold" -Recurse -File | ForEach-Object {
  $rel = $_.FullName.Substring("$SKILL_DIR\scaffold\".Length) -replace '\.txt$',''
  $dest = Join-Path $SCRATCH $rel
  New-Item -ItemType Directory -Force -Path (Split-Path $dest) | Out-Null
  Copy-Item $_.FullName $dest -Force
}
```
Then run the same `npm install` / `npm pkg set` commands. ffmpeg comes from
